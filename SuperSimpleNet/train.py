...
def main_our_dataset(device, config):
    config = copy.deepcopy(config)
    config["dataset"] = config.get("dataset_name", "dataset_resized_1536x2048")
...
def main_our_dataset_defect_synthesis(device, config):
    config = copy.deepcopy(config)
    config["dataset"] = "defected_red_yellow"
    config["category"] = "stitch"
    config["name"] = f"{config['category']}_{config['setup_name']}"

    print(f"Training on custom dataset with resolution: {config['image_size']}")

    model = SuperSimpleNet(image_size=config["image_size"], config=config)

    datamodule = SupervisedStitchDataModule(
        root=Path(config["datasets_folder"]) / "defected_red_yellow",
        image_size=config["image_size"],
        batch_size=config["batch"],
        num_workers=config["num_workers"]
    )
